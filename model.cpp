...
void ModelLoader::save_to_gguf(const std::string& file_path,
                               const char* tensor_type_rules_str,
                               const std::map<std::string, ggml_type>& file_to_wtype,
                               ggml_type new_type,
                               int n_threads) {
    ...
    auto on_new_tensor_cb = [&](const TensorStorage& tensor_storage, ggml_tensor** dst_tensor) -> bool {
        ...
        if (!set) {
            ...
            auto it                      = file_to_wtype.find(file_path);
            if (it != file_to_wtype.end()) {
                dst_type = it->second;
            }
        }
