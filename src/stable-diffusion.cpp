...
SD_API bool generate_image(sd_ctx_t* sd_ctx,
    ...
    const RefImageParams ref_image_params = sd_ctx->sd->resolve_ref_image_params(sd_img_gen_params->ref_image_args);
        ...
        auto latents_opt = prepare_image_generation_latents(sd_ctx,
                                                            ...
                                                            &request,
                                                            &plan,
        ref_image_params);if (!latents_opt.has_value()) {
            return false;
        }
        ...
        auto embeds_opt = prepare_image_generation_embeds(sd_ctx,
                                                          ...
                                                          &plan,
                                                          &latents,
        ref_image_params);if (!embeds_opt.has_value()) {
            return false;
        }
...
SD_API bool generate_video(sd_ctx_t* sd_ctx,
    ...
    const RefImageParams ref_image_params;    int64_t t0                    = ggml_time_ms();
        sd_ctx->sd->vae_tiling_params = sd_vid_gen_params->vae_tiling_params;apply_circular_axes_to_diffusion(sd_ctx, sd_vid_gen_params->circular_x, sd_vid_gen_params->circular_y);
