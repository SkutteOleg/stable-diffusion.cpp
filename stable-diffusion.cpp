...
sd_image_t* generate_image(sd_ctx_t* sd_ctx, const sd_img_gen_params_t* sd_img_gen_params) {
    try {
        ...
        int vae_scale_factor          = sd_ctx->sd->get_vae_scale_factor();if (sd_version_is_dit(sd_ctx->sd->version)) {
    ...
    sd_ctx->sd->sampler_rng->manual_seed(seed);
        ...
        if (sd_img_gen_params->init_image.data) {
            ...
            if (sd_version_is_inpaint(sd_ctx->sd->version)) {
                ...
                init_latent   = sd_ctx->sd->encode_first_stage(work_ctx, init_img);} else {
                    // mask after vae
                    init_latent   = sd_ctx->sd->encode_first_stage(work_ctx, init_img);masked_latent = ggml_new_tensor_4d(work_ctx, GGML_TYPE_F32, init_latent->ne[0], init_latent->ne[1], init_latent->ne[2], 1);
...
SD_API sd_image_t* generate_video(sd_ctx_t* sd_ctx, const sd_vid_gen_params_t* sd_vid_gen_params, int* num_frames_out) {
    ...
    sd_ctx->sd->sampler_rng->manual_seed(seed);
        ...
        sd_ctx->sd->lora_stat();sd_image_t* result_images = (sd_image_t*)calloc(vid->ne[2], sizeof(sd_image_t));
        if (result_images == nullptr) {
            ggml_free(work_ctx);
            return nullptr;
        }
