...
public:
    ...
    void init_backend() {
        ...
        if (const char* s = getenv("GGML_VK_DEVICE")) {
            ...
            for (int i = 0; i < dev_count; i++) {
                ...
                // "AMD Radeon(TM) Graphics" is the typical name for APUs/iGPUs
                if (d.find("Radeon") != std::string::npos && d.find("Graphics") != std::string::npos &&
                    d.find("RX") == std::string::npos && d.find("Pro") == std::string::npos) {
                    score -= 1LL << 40; // Remove the discrete GPU bonus for likely integrated Radeon graphics
                }
    ...
    ggml_tensor* sample(ggml_context* work_ctx,
        ...
        if (!sample_k_diffusion(method, denoise, work_ctx, x, sigmas, sampler_rng, eta, ge_gamma)) {
