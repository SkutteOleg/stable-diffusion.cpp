...
SDError new_sd_ctx(sd_ctx_t** sd_ctx,
...
sd_image_t* generate_image(sd_ctx_t* sd_ctx,
    ...
    try {
        ...
        if (sd_ctx->sd->stacked_id) {
            ...
            bool pmv2 = sd_ctx->sd->pmid_model->get_version() == PM_VERSION_2;if (sd_ctx->sd->pmid_model && input_id_images_path.size() > 0) {
    ...
    if (sd_version_is_inpaint(sd_ctx->sd->version)) {
        ...
        for (int b = 0; b < batch_count; b++) {
            ...
            skip_layers,
                                                     ...
                                                     skip_layer_start,
                                                     skip_layer_end,
                                                     noise_mask);// struct ggml_tensor* x_0 = load_tensor_from_file(ctx, "samples_ddim.bin");
        ...
        if (result_images == NULL) {
            LOG_ERROR("Failed to allocate memory for result images");
            return NULL;
        }
        ...
        for (size_t i = 0; i < decoded_images.size(); i++) {
            ...
            if (result_images[i].data == NULL) {
                LOG_ERROR("Failed to convert tensor to image for result %zu", i);
                for (size_t j = 0; j < i; j++) {
                    free(result_images[j].data);
                }
                free(result_images);
                return NULL;
            }
        ...
        LOG_ERROR("Exception in generate_image: %s", e.what());
        return NULL;
    } catch (...) {
        LOG_ERROR("Unknown exception in generate_image");
        return NULL;
    }
}

SDError txt2img(sd_ctx_t* sd_ctx,
